    ...
    <div>
        ...
        <nav class="secondary-sidebar">
            <div class=" m-b-30 m-l-30 m-r-30 hidden-sm-down">
                <a href="#" class="btn btn-complete btn-block btn-compose" @click="viewformReport()">Created Report</a>
            </div>
            <p class="menu-title">REPORT ERROR</p>
            <ul class="main-menu">
                <li @click="getdataReport(0)" :class="{ active : active_el == 0 }">
                    <a href="#">
                        <span class="title"><i class="pg-inbox"></i> New</span>
                        <span class="badge pull-right">{{ countReport.new }}</span>
                    </a>
                </li>
                <li @click="getdataReport(1)" :class="{ active : active_el == 1 }">
                    <a href="#">
                        <span class="title"><i class="pg-layouts4"></i> On Progress</span>
                        <span class="badge pull-right">{{ countReport.onpro }}</span>
                    </a>
                </li>
                <li @click="getdataReport(2)" :class="{ active : active_el == 2 }">
                    <a href="#">
                ...
                <div class="timeline-block m-0 m-b-15" v-for="lists in temp" track-by="$index/id_report">
                    ...
                    <div class="timeline-content bounce-in m-l-0 p-b-10">
                        <div class="card social-card share full-width">
                            ...
                            <div class="card-description" v-html="lists.notes"> <!-- isi notes -->
        ...
        <Form_userReport :form_report='form_Report_view' :class="{ hide : checHide == 1 }"></Form_userReport>
...
<script>
    let Form_userReport = require('../components/Formreport.vue'); // form report

    export default{
        components:{Form_userReport},
        data(){
            return {
                userReport: {},
        ...
        watch:{
            searchQuery(){
                if(this.searchQuery.length > 0){
                    this.temp =  this.userReport.filter((item) => {
        ...
        created:function () {
            ...
            let self = this;
            this.$bus.$on('getdataReport', function(data){
                console.log(data);
                self.getdataReport(data)
            })
        },
        methods:{
            getdataReport(idStatus){
                var self = this;
                axios.get(`../../api/report/user/getdata?id_status=${idStatus}`)
                    .then((response) => {
                        self.userReport = self.temp = response.data.userReport; // data report
                        ...
                        self.checHide = 1; // show and hide
                    });
            },
            viewformReport(){
                var self = this
                self.active_el = ''; // add class active
                self.checHide = 0; // show and hide
            }
        }
    }
